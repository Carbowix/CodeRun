...
interface SnippetChangeTitleProps {
  title: string;
  snippetId: string;
  guestMode: boolean;
}
export default function SnippetChangeTitle({
  title,
  snippetId,
  guestMode,
}: SnippetChangeTitleProps) {
  ...
  const handleEdit = async (event: FormEvent) => {
    ...
    if (currentTitle.trim() !== '') {
      const response = await fetch('/api/snippet/editTitle', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
        body: JSON.stringify({
          snippetTitle: currentTitle,
          snippetId: snippetId,
        }),
      });

      if (response.status == 200) {
        toast.success("Successfuly edited snippet's title");
      } else {
